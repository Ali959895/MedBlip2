...
def plot_models_by_class_9x6(
    ...
    cell_w = 2.2
    cell_h = 2.6
    # less left padding now that labels are rotated
    fig_w = n_cols * cell_w + 0.55
    ...
    plt.subplots_adjust(
        left=0.02, right=0.995, top=0.95, bottom=0.05,
...
#        axes[0, j].set_title(col_title, fontsize=12, fontweight="bold", pad=10)
    ...
    x_text = 0.008  # move left/right here (smaller => closer to edge)
    for i, m in enumerate(model_names):
...
def main():
    ...
    ap.add_argument("--npz_dir", required=True, help="Directory containing test_preds_*.npz for models.")
    ap.add_argument("--manifest", required=True, help="CSV manifest for test set (image paths).")
    ap.add_argument("--root", required=True, help="Root dir to resolve relative image paths in manifest.")
    ap.add_argument("--outdir", required=True, help="Output directory.")
    ap.add_argument("--class_names", default="", help="Comma-separated class names (index order).")
    ap.add_argument("--models", default="", help="Comma-separated model stems to include (optional).")
    ap.add_argument("--class_ids", default="", help="Comma-separated 6 class indices, e.g. '0,1,2,3,4,5'.")
    ap.add_argument("--thumb", type=int, default=256)
    ap.add_argument("--dpi", type=int, default=300)
    ap.add_argument("--title", default="", help="Optional title.")
    ap.add_argument("--seed", type=int, default=0,
                help="Random seed for selecting different images per class each run.")
