...
class BlipBase(BaseModel):
    ...
    def init_tokenizer(cls):
        tokenizer = BertTokenizer.from_pretrained("/scratch/ali95/.cache/huggingface/hub/models--bert-base-uncased/snapshots/1234567890abcdef", local_files_only=True)
    ...
    def load_from_pretrained(self, url_or_filename):
        ...
        # LAVIS checkpoints often: {"model": state_dict, ...}
        # HF checkpoints often: plain state_dict (OrderedDict)
        if isinstance(checkpoint, dict) and "model" in checkpoint:
            state_dict = checkpoint["model"]
        elif isinstance(checkpoint, dict) and "state_dict" in checkpoint:
            state_dict = checkpoint["state_dict"]
        else:
            # assume checkpoint itself is already a state_dict
